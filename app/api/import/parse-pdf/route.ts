...
import { extractInvoiceFromSources } from "@/lib/anthropic/extract-invoice";
...
export async function POST(req: NextRequest) {
  ...
  try {
    const { extracted, usage } = await extractInvoiceFromSources(
      validated.sources,
      validated.multiPageImage,
    );

    console.log(
      `[OCR] ${files.map((f) => f.name).join(", ")} (${files.length} str.): in ${usage.input_tokens} / out ${usage.output_tokens} tokens`,
    );
